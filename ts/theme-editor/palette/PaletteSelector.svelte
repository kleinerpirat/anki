...
<script lang="ts">
    ...
    import type { ConfigListEntry, PaletteEditorState } from "./lib";
    ...
    const dispatchPaletteChange = () => dispatch("palettechange");

    $: value = $configList.findIndex((entry) => entry.current);
    $: label = paletteLabel($configList[value]);

    function paletteLabel(entry: ConfigListEntry): string {
    ...
    <ButtonToolbar class="justify-content-between flex-grow-1" wrap={false}>
        <Select class="flex-grow-1" bind:value {label} on:change={blur}>
            {#each $configList as entry}
                <SelectOption value={entry.idx}>{paletteLabel(entry)}</SelectOption>
            {/each}
        </Select>
        ...
        <SaveButton
            ...
            on:remove={dispatchPaletteChange}
        />
    </ButtonToolbar>
</StickyContainer>
